...
**Unique feature**
- **R8 auto equip** - Auto equip R8 revolver on new login accounts with EYA tokens.

**Two Login Modes**

- **Manual mode** - Enter an account username and EYA token, then log in to the local Steam client with one single click.
- **Automatic mode** (Currently support 3 China account selling websites ONLY) - Enter a card key, then automatically resolve the username and EYA token from the server.
